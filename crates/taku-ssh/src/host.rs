...
impl Host {
    ...
    fn from_target(target: &str) -> Host {
        ...
        let (host, port) = Host::split_host_port(rest);
        Host {
            host,
            user,
            port,
    ...
    fn from_table(t: &Table) -> mlua::Result<Host> {
        let host: Option<String> = t.get("host")?;
    ...
    pub(crate) fn ssh_base(&self) -> Command {
        ...
        // User options first: ssh uses the first occurrence of an option, so
        // these win over the multiplexing defaults below.
        for opt in &self.options {
            cmd.arg("-o").arg(opt);
        }
        // Multiplex connections: the first command opens a master that later
        // commands reuse for a while (ControlPersist), so a run of remote
        // operations doesn't pay a full handshake + auth each. Not supported
        // by the Windows OpenSSH port.
        #[cfg(not(windows))]
        {
            cmd.arg("-o").arg("ControlMaster=auto");
            cmd.arg("-o").arg(format!(
                "ControlPath={}",
                std::env::temp_dir().join("taku-ssh-%C").display()
            ));
        ...
        if let Some(password) = &self.password {
            ...
            if let Some(helper) = &self.askpass {
                ...
                // ssh only runs the askpass helper when DISPLAY is set; a
                // dummy value keeps password auth working on headless
                // machines. We still don't set SSH_ASKPASS_REQUIRE, so with a
                // tty ssh's own prompts (password, known_hosts) win.
    ...
    /// Splits a trailing `:port` off a target. `[v6]:port` brackets are
    /// honoured, and a bare IPv6 address (more than one `:`) is left intact
    /// rather than having its last group misread as a port.
...
mod tests {
    ...
    #[test]
    fn bare_ipv6_is_not_split_at_its_last_colon() {
        let h = Host::from_target("user@::1");
    ...
    #[test]
    fn bracketed_ipv6_takes_a_port() {
        let h = Host::from_target("user@[2001:db8::1]:2222");
        assert_eq!(h.host, "2001:db8::1");
        assert_eq!(h.port, Some(2222));
        let h = Host::from_target("[::1]");
        assert_eq!(h.host, "::1");
        assert!(h.port.is_none());
    }

    #[test]
    fn destination_follows_a_double_dash() {
        let cmd = Host::from_target("-evil@host").ssh_command("uptime");
        let args: Vec<_> = cmd
            .get_args()
            .map(|a| a.to_string_lossy().into_owned())
            .collect();
        let dashdash = args.iter().position(|a| a == "--").expect("no -- in argv");
        assert_eq!(args[dashdash + 1], "-evil@host");
    }
