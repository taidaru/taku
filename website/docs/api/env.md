...
If a `.env` file sits next to your `Takefile.lua`, Taku loads it automatically and
...
`\n`, `\t`, `\r`, `\\`, and `\"` escapes are expanded). The file is never written
to and is only read on this machine.
